...
object Ast {
  case class identifier(name: String) extends Ast {
    override def render(indent: Int = 0) = name
  }
  ...
  object stmt {
    ...
    case class Assign(targets: Seq[expr], value: expr) extends stmt {
      override def render(indent: Int = 0): String = {
        val pat = targets.map(_.render(indent))
        (if (pat.size == 1) pat.head else pat.mkString("(", ",", ")")) + " = " + value.render(indent)
      }
    }
  ...
  object expr {
    ...
    case class Num(n: Any) extends expr {
      override def render(indent: Int = 0): String = n.toString
    }
    ...
    case class Name(id: identifier, ctx: expr_context) extends expr {
      override def render(indent: Int = 0): String = ctx match {
        case expr_context.Load => id.render(indent)
      }
    }
  ...
  sealed trait expr_context extends Ast
  ...
  sealed trait slice extends Ast
  ...
  sealed trait boolop extends Ast
  ...
  sealed trait unaryop extends Ast
  ...
  sealed trait cmpop extends Ast
  ...
  case class comprehension(target: expr, iter: expr, ifs: Seq[expr]) extends Ast

  // not sure what to call the first argument for raise and except
  sealed trait excepthandler extends Ast
  ...
  case class alias(name: identifier, asname: Option[identifier]) extends Ast
}